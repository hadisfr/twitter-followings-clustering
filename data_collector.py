...
def collect_followings_relations(starting_user_id, max_level, tweepy_api):
    ...
    for level in range(max_level):
        ...
        for user in last_stage:
            index += 1
            print(("\r\tproccessing %d of %d (%.2f%%)..." + clear_string * 3 + "\r") % (index, len(last_stage), index / len(last_stage) * 100), end = "", file = sys.stderr)
            sys.stderr.flush()
            user_followings = []
            while True:
                try:
        ...
        print(("\rstage size: %d" + clear_string * 5 + "\n") % (len(stage)), file = sys.stderr)
    ...
    for user in last_stage:
        ...
        for following in user_followings:
            ...
            if following in stage:
                if not last_level_max_followings_filter and not max_followings_filter:
                    followings[user].append(following)
                else:
                    followings_number = max_followings_filter
                    try:
    ...
    print(("\rcollecting data completed. stage size: %d" + clear_string + "\n") % (len(stage)), file = sys.stderr)
